      ...
      <div className="flex items-center justify-between gap-2">
        {sticherHeaderOptions.slice(0, -2).map((option) => (
          <button
            ...
            style={{
              backgroundColor: activeHeaderOption === option.id ? '#E6F4FF' : '',
              borderColor: activeHeaderOption === option.id ? palette.primaryLight : '#d1d1d1',
            }}
          >
            <option.icon size={option.iconSize || 22} color={activeHeaderOption === option.id ? palette.primary : '#707070'} />
            <span
              className="ml-2 text-sm font-semibold"
              style={{
                color: activeHeaderOption === option.id ? palette.primary : '#707070',
              }}
            >
              {option.title}
            </span>
